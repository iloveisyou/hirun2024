...
Rui.extend(Rui.ui.LUnorderedList, Rui.ui.LUIComponent, {
    ...
    markParents:function(isMarked, node){
        ...
        /**
     * DataSet row 위치 마크 및 recursive 호출
     * @private
     * @method markData
     * @param {int} row
     * @param {bool} isSelect
    ...
    /**
     * @description dataSet의 onMarked event 처리, 재귀동작 처리를 위해 onMarked 이벤트 대신 동작
     * @method onMark
     * @private
     * @return {void}
     */
...
/**
 * @description 트리를 출력하는 콤포넌트
 * @module ui_tree
 * @title LTreeView
 * @requires Rui.ui.LUnorderedList
 */
Rui.namespace('Rui.ui.tree');
/**
 * 트리를 출력하는 객체
 * @namespace Rui.ui.tree
 * @class LTreeView
 * @extends Rui.ui.LUnorderedList
 * @sample default
 * @plugin /ui/tree/rui_tree.js,/ui/tree/rui_tree.css
 * @constructor LTreeView
 * @param {Object} config The intial LTreeView.
 */
...
Rui.extend(Rui.ui.tree.LTreeView, Rui.ui.LUnorderedList, {
     ...
     * @config toggleByLabel
     * @type {boolean}
     * @default false
     */
    /**
     * @description label click시 toggle할지 여부
     * @property toggleByLabel
     * @type {boolean}
     * @default false
     */
    toggleByLabel: false,   
    ...
    /**
     * @description li 내부의 div의 내용 return
     * @private
     * @method getContent
     * @param {Rui.data.LRecord} record content 정보 record
    ...
    getContent: function(record,nodeStateClass,isMarked){
        ...
        /*
        if(this.showFolder && hasChildNode){
            folderImageClass = ' ' + folderImageClass;
            folderHTML = '<td class="' + this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL + " " + this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL_FOLDER+'"><div class="' + this.CLASS_UL_TREEVIEW_NODE_FOLDER + folderImageClass + '" style="">&nbsp;</div></td>';//&nbsp;
        } else {
            folderHTML = '<td class="' + this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL + " " +this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL_FOLDER_HIDDEN+'"><div class="' + this.CLASS_UL_TREEVIEW_NODE_FOLDER + folderImageClass + '" style="">&nbsp;</div></td>';//&nbsp;
        }
        */
        ...
        var checkboxClass = '';
        if (this.nodeType.toLowerCase() == 'checkbox') {
            checkboxClass = this.CLASS_UL_TREEVIEW_NODE_CHECKBOX;
            if(isMarked){
                checkboxClass += ' ' + this.CLASS_UL_TREEVIEW_NODE_CHECKBOX_CHECKED;
            } else {
                checkboxClass += ' ' + this.CLASS_UL_TREEVIEW_NODE_CHECKBOX_UNCHECKED;
            }
            checkboxHTML = '<td class="' + this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL + " " +this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL_CHECKBOX+'"><div class="' + checkboxClass + '" style="">&nbsp;</div></td>';
        } else {
            checkboxHTML = '<td class="' + this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL + " " +this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL_CHECKBOX_HIDDEN+'"><div class="' + checkboxClass + '" style="">&nbsp;</div></td>';
        }*/
        ...
        /*
        label = '<table cellspacing="0" cellpadding="0" border="0" class="' + this.CLASS_UL_TREEVIEW_NODE_TABLE + '"><tbody><tr>'
                +'<td class="'+this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL+'"><div class="' + this.CLASS_UL_TREEVIEW_NODE_IMAGE + ' ' + nodeImageClass + '" style="">&nbsp;</div></td>' 
                + folderHTML 
                + checkboxHTML 
                + '<td class="'+this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL+ ' ' +this.CLASS_UL_TREEVIEW_NODE_TABLE_CELL_CONTENT+'"><div class="' + this.CLASS_UL_TREEVIEW_NODE_CONTENT + '">' 
                    + '<a class="' + this.CLASS_UL_TREEVIEW_NODE_LABEL + '">' + label + '</a>' 
                + '</div></td>'
                + '</tr></tbody></table>'; */
        return label;
    ...
    /**
     * @description node click시 실행되는 내용, 호환이 필요 없으면 eventType으로 식별.
     * @method onNodeClick
     * @private
     * @param {HTMLElement} _div click한 node div
     * @param {String} rId click한 menu object의 record id
     ...
     * @method createNodeObject
     * @param {HTMLElement} dom
     * @return {Rui.ui.tree.LTreeViewNode}
     */
    createNodeObject: function(dom){
 ...
 * @constructor LTreeViewNode
 * @param {Object} config The intial LTreeViewNode.
 * @plugin /ui/tree/rui_tree.js,/ui/tree/rui_tree.css
 */
...
Rui.extend(Rui.ui.tree.LTreeViewNode, Rui.ui.LUnorderedListNode, {
    ...
    /**
     * part type으로 content를 구성하는 부분들을 지칭한다.
     * @private
     * @property partType
     * @type {object}
     */
    ...
    /**
     * content div에서 특정 part가져오기
     * @private
     * @method getPartEl
     * @param {object} partType struct
     * @param {HTMLElement} dom optional
     * @return {void}
     */
