Rui.ui.guide.Dashboard001 = function(config) {
    Rui.ui.guide.Dashboard001.superclass.constructor.call(this, config);
}

Rui.extend(Rui.ui.guide.Dashboard001, Rui.ui.LGuide, {
    onReady: function() {
        this.loadPage();
        this.focusGrid1();
        this.focusGrid2();
    ...
    doLoadPage: function() {
         ...
         var focusNotification1 = new Rui.ui.LFocusNotification({ 
             text: 'RUI 기능의 튜토리얼을 시작합니다. 페이지별/로그인 아이디(demo1/demo2)별 튜토리얼이 다르게 작동합니다.<br><br>화면 상단에 보이시는 차트는 오픈 소스 차트입니다. 모든 브라우저에서 작동하는 차트로 다양한 OS와 브라우저에서 잘 작동됩니다. 하단의 각 그리드들을 한번씩 클릭해보세요.',
         ...
         focusNotification1.on('closed', function(e) {
             ...
             var focusNotification2 = new Rui.ui.LFocusNotification({ 
                 text: '주문 현황 그리드에는 트리그리드가 적용되어 있습니다. 접혀있는 row를 펼쳐보세요.',
                 view: r, dialog: dialogRegion }
             );
    ...
    focusGrid2: function() {
        ...
        if(this.getInt('doColumnresize') == 0) {
            var columnModel2 = this.params.columnModel2;
            var column = columnModel2.getColumnAt(3);
            column.on('columnResize', function(e) {
                Rui.later(1000, this, this.doColumnresize);
            }, this, true);
        }
    },
    doFocusGrid2: function() {
        var grid2 = this.params.grid2;
        var view = grid2.getView();
        var r = Rui.get(view.getHeaderCellEl(3)).getRegion();
        r.left = r.right - 10;
        r.right = r.right + 10;
        var dialogRegion = Rui.util.LObject.clone(r);

        dialogRegion.top += 50;
        dialogRegion.left -= 45;
    ...
    doColumnresize: function() {
        var grid2 = this.params.grid2;
        var view = grid2.getView();
        var r = Rui.get(view.getHeaderCellEl(1)).getRegion();
        r.left = r.left + 40;
        r.right = r.right - 40;
        var dialogRegion = Rui.util.LObject.clone(r);

        dialogRegion.top += 50;
        dialogRegion.left -= 80;
        dialogRegion.right += 100;
        dialogRegion.bottom = dialogRegion.top + 100;

        new Rui.ui.LFocusNotification({ 
            text: '헤더 컬럼 타이틀을 클릭하면 데이터가 정렬됩니다.',
            view: r, dialog: dialogRegion  
        } ).render(document.body);
    },
    focusGrid3: function() {
